...
def train_task_for_data(cfg, **kwargs):
    ...
    # ##创建经过筛选后的数据加载器
    tmp_path = '/home/zjy/lmy/UnlearningDiffusion/test/param_data/tmp_2024-08-30_15-12-35'
    his_path = tmp_path + '/'
    
    clients[0].net = torch.load(his_path + 'u0_start.pth')

    clients[1].net = torch.load(his_path + 'u1_continue_learning.pth')
    clients[2].net = torch.load(his_path + 'u2_continue_learning.pth')
    clients[3].net = torch.load(his_path + 'u3_continue_learning.pth')
