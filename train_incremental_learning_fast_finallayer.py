...
def train_task_for_data(cfg, **kwargs):
    ...
    # 要删除的类别列表
    sense_classes = list(range(15))  # 类别0到10
    num_clients = 5  
    ...
    # clients[1].train_continue(epoch, all_epoch,aggre_wei=[u0], name='u1_continue_learning', cfg=cfg)
    # u1 = clients[1].net
    # clients[2].train_continue(epoch, all_epoch, aggre_wei=[u1],name='u2_continue_learning', cfg=cfg)
    # u2 = clients[2].net
    # clients[3].train_continue(epoch, all_epoch,aggre_wei=[u2],name='u3_continue_learning', cfg=cfg)
    # u3 = clients[3].net
    ...
    # ##创建经过筛选后的数据加载器
    tmp_path = '/home/zjy/lmy/UnlearningDiffusion/test/param_data_his/tinyimagenet_incre_densenet161'
    his_path = tmp_path + '/'
    
    clients[0].net = torch.load(his_path + 'u0_start.pth').to('cuda:7')

    clients[1].net = torch.load(his_path + 'u1_continue_learning.pth').to('cuda:7')
    clients[2].net = torch.load(his_path + 'u2_continue_learning.pth').to('cuda:7')
    clients[3].net = torch.load(his_path + 'u3_continue_learning.pth').to('cuda:7')
