...
def train_task_for_data(cfg, **kwargs):
    ...
    for i in range(num_clients):
        ...
        # if i == 1:
        #     # 创建经过筛选后的数据加载器
        #     train_loader, eval_loader = remove_sense_get_loader(dataset, test_dataset, [70, 71,72,73,74], batch_size, num_workers)
        # if i == 2:
        #     # 创建经过筛选后的数据加载器
        #     train_loader, eval_loader = remove_sense_get_loader(dataset, test_dataset, [60,61,62,63,64], batch_size, num_workers)
    ...
    # ##创建经过筛选后的数据加载器
    tmp_path = '/home/zjy/lmy/UnlearningDiffusion/test/exp_result_cifar100/resnet18/param_data_fed/tmp_2024-06-24_10-16-01'
    his_path = tmp_path + '/'
    
    r1 = torch.load(his_path + 'r1.pth').to('cuda:7')

    r2 = torch.load(his_path + 'r2.pth').to('cuda:7')
    r3 = torch.load(his_path + 'r3.pth').to('cuda:7')
    r4 = torch.load(his_path + 'r4.pth').to('cuda:7')
    ...
    #ssd_graph_tuning(cfg,[r1],[clients[1]], [r2,r3,r4], [clients[1],clients[2],clients[2]], next(clients[0].net.parameters()).device, clients[0].train_layer, classes_to_remove)
    finetune(cfg,[r1],[clients[1]], [r2,r3,r4], [clients[1],clients[2],clients[2]], next(clients[0].net.parameters()).device, clients[0].train_layer, classes_to_remove)
