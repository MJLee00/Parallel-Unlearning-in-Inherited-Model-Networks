...
def train_task_for_data(cfg, **kwargs):
    ...
    filtered_indices = [i for i, x in enumerate(train_dataset.targets) if x == 0 or x == 1]
    filtered_data = train_dataset.data[filtered_indices]
    filtered_targets = [train_dataset.targets[i] for i in filtered_indices]

    # 进行 train-test split
    client_data, test_client_data, client_targets, test_client_targets = train_test_split(
        filtered_data, filtered_targets, test_size=0.2, random_state=42
    )
    ...
    # ##创建经过筛选后的数据加载器
    #tmp_path = '/home/zjy/lmy/UnlearningDiffusion/test/param_data/tmp_2024-11-07_21-32-38'
    his_path = tmp_path + '/'
    
    r1 = torch.load(his_path + 'd.pth').to('cuda:7')

    r2 = copy.deepcopy(r1).to('cuda:7')
    r3 = copy.deepcopy(r1).to('cuda:7')
    r4 = copy.deepcopy(r1).to('cuda:7')
    ...
    ssd_graph_tuning_multi(cfg,[r1],[clients[0]], [r2,r3,r4], [clients[0],clients[0],clients[0]], next(clients[0].net.parameters()).device, clients[0].train_layer)
