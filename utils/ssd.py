...
class ParameterPerturber:
    def __init__(
        ...
        self.train_layer = parameters['train_layer']
    ...
    def calc_importance(self, dataloader: DataLoader) -> Dict[str, torch.Tensor]:
        ...
        fix_partial_model(self.train_layer, self.model)
        forward_times, backward_times = [], []

        for batch_idx, (x, y) in enumerate(dataloader):
            if str(type(self.model)) == "<class 'models.gcnn.GCNN'>":
                x, y = x.to(self.device), y.to(self.device)
            elif str(type(self.model)) == "<class 'models.bert.Bert'>":
                max_length = min(max([len(text) for text in x]) + 2, 128)
                batch_input_ids, batch_att_mask = [], []
                for text in x:
                    encoding = self.model.get_tokenizer().encode_plus(
                        text,
                ...
                x = torch.cat(batch_input_ids).to(self.device)
                att_mask = torch.cat(batch_att_mask).to(self.device)
                y = y.to(self.device)
            else:
                x, y = x.to(self.device).float(), y.to(self.device)
            if self.test and str(type(self.model)) != "<class 'models.gcnn.GCNN'" \
                    and str(type(self.model)) != "<class 'models.bert.Bert'>":
                x = x[:32, :, :, :]
                y = y[:32]
            ...
            if str(type(self.model)) == "<class 'models.bert.Bert'>":
                out = self.model(x, att_mask)
            else:
                out = self.model(x)
        ...
        # Print average times
        print(f"Average forward pass time: {sum(forward_times) :.4f} seconds")
        print(f"Average backward pass time: {sum(backward_times):.4f} seconds")
    
        return importances

    def calc_rl_importance(self, s,y,r, batch_size=128) -> Dict[str, torch.Tensor]:
        criterion = nn.CrossEntropyLoss()
        importances = self.zerolike_params_dict(self.model)
        fix_partial_model(self.train_layer, self.model)
        permutation = np.random.permutation(s.size(0))
        for i in range(0, s.size(0), batch_size):
            indices = permutation[i:i + batch_size]
            batch_states = s[indices]
            batch_actions = y[indices]
            batch_rewards = r[indices]
            time_step = torch.arange(0, batch_size).type(torch.long).to('cuda:7')
            
            b_s, b_a, b_r = batch_states.to(self.device).float(), batch_actions.to(self.device), batch_rewards.to(self.device)
            if b_s.shape[0] != batch_size:
                continue
            self.opt.zero_grad()
            out = self.model(b_s, b_a, b_r, time_step)
            loss = criterion(out, b_a)
            loss.backward()
    ...
    def reverse_modify_weight(
        ...
        with torch.no_grad():
            for (n, p), (oimp_n, oimp), (fimp_n, fimp) in zip(
                ...
                # Synapse Dampening with parameter lambda
                weight = ((fimp.mul(self.dampening_constant)).div(oimp)).pow(
                    self.exponent
                )
                update = weight[locations]
                # Bound by 1 to prevent parameter values to increase.
